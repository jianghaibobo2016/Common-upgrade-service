...
enum PC_DEV_COMMAND {
	...
	CMD_DEV_REQUESTCASTMODE = 10		  //获取单播组播模式
...
typedef struct DEV_Reply_GetDevMsg_tag {
	...
	INT8 DevMACAddress[13];  //设备MAC地址 //ASCII字符大写，包含结束符
	INT8 DevID[40];          //设备ID(前四个字符为设备类型)
	INT8 DevType[20];        //设备型号
	INT8 HardVersion[20];    //硬件版本
	INT8 SoftVersion[20];    //软件版本
	INT8 DevName[60];  	 	 //设备名称;
...
typedef struct PC_Request_ParameterSetting_tag {
	PC_DEV_Header header;
	INT8 DevID[40];          //设备ID(前四个字符为设备类型) //2018.05.23 add
...
//------------------------------10. 获取单播组播模式------------------------//
//设备返回
typedef struct DEV_CastMode_tag {
	PC_DEV_Header header;
	INT8 DevID[40]; //设备ID(前四个字符为设备类型)//add
