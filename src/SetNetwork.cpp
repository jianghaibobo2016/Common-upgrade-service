...
SetNetworkTerminal::SetNetworkTerminal() :
		m_netWorkConfig(), IFNAME(NULL), networkStatus(errorStatus) {
}
//???
SetNetworkTerminal::SetNetworkTerminal(
...
bool SetNetworkTerminal::getNetworkConfig() {
	INT32 sock;
...
INT8* SetNetworkTerminal::castMacToChar13(INT8 *macDest, string macaddr) {
...
bool SetNetworkTerminal::setNetworkConfig(const INT8 *ipaddr,
		const INT8 *subnet, const INT8 *gateway, const INT8* macaddr,
		const INT8* iniFile) {
...
bool SetNetworkTerminal::setNet(const INT8 *ipaddr, const INT8 *subnet,
		const INT8 *gateway) {
...
bool SetNetworkTerminal::setNet(INT32 mac, const INT8 *macaddr) {
	struct ifreq temp;
...
string SetNetworkTerminal::ByteToHexString(const void *pData, INT32 len) {
	const string &split = "";
	INT8 strNum[100];
	string strResult;
...
bool SetNetworkTerminal::upDownNetwork(NetworkStatus networkStatus) {
	INT32 fd, rtn;
	struct ifreq ifr;
...
bool CheckNetConfig::checkMAC(INT8 *mac) {
	...
	// if (lastBit & 1)
	// {
	//     Logger::GetInstance().Error("Error mac adddress of *%c !", strtmp[1]);
	//     return false;
	// }
	INT32 hexnum = 0;
	while (strtmp != NULL) {
...
// IniConfigFile::IniConfigFile(INT8 *iniFile)
//     : iniFile(iniFile)
// {}
...
INT32 SetNetworkTerminal::stringToHex(const string &strNum) {
	if (strNum.length() != 2) {
...
INT32 SetNetworkTerminal::charToHex(const char cNum) {
	if (cNum >= '0' && cNum <= '9') {
		return cNum - '0';
...
bool SetNetworkTerminal::cSplitString(const string strKey, const string strChar,
		vector<string> &vMac) {
